...
class PINN(nn.Module):
    def __init__(self, input_dim=3, output_dim=2, hidden_dim=50, num_hidden=3, activation='sin'):
        ...
        self.q = nn.Parameter(torch.tensor([q], requires_grad=True).float())
    ...
    def loss_PDE(self, x, y, t):
        ...
        # PDE residual
        residual = (DENSITY * SPECIFIC_HEAT_CAPACITY * u_t) - (self.k* u_xx) - (self.k * u_yy) - self.q
...
def closure():
    ...
    print(f'Epoch {model.epoch}, loss = {loss.item():.4e},  k = {model.k.item():.6f}, q = {model.q.item():.6f}')
