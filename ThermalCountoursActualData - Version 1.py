...
TempXColumns = ["X-150", "X-10","X-5","X-2.5","X-1","X00", "X1","X2.5","X5","X10","X150"]
TempYColumns = ["Y00", "Y01","Y1","Y2.5","Y5","Y10","Y150"]

data = pd.read_csv(
    "readings.csv", sep=',',
    names=["Timestamp", "Power", "X00", "X01","X1","X2.5","X5","X10","X150","X151",
           "Y00", "Y01","Y1","Y2.5","Y5","Y10","Y150","Y151"]
)

data["X-1"] = data["X1"]
data["X-2.5"] = data["X2.5"]
data["X-5"] = data["X5"]
data["X-10"] = data["X10"]
data["X-150"] = data["X150"]
data["X-151"] = data["X151"]
data["Y-1"] = data["Y1"]
data["Y-2.5"] = data["Y2.5"]
data["Y-5"] = data["Y5"]
data["Y-10"] = data["Y10"]
data["Y-150"] = data["Y150"]
data["Y-151"] = data["Y151"]

XData = np.array([-15,-10,-5,-2.5,-1,0,1,2.5,5,10,15])  # shape = (11,)
YData = np.array([0,1,2.5,5,10,15])                   # shape = (6,)
...
def fetchData(t):
    ...
    for i in range(6):
        newData = []
        TempYData = data[TempYColumns[i]][idx]
        for j in range(11):
            TempXData = data[TempXColumns[j]][idx]
            newData.append((TempXData + TempYData) / 2)
        if TempData is None:
            TempData = np.array(newData)
        else:
            TempData = np.vstack((TempData,newData))
